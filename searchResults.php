<?php
  ...
  if ($hata == false && isset($_POST['btn_like']) && isset($_SESSION['userId'])) { // Like durumu
    ...
    if (isContentLiked($content_id, $_SESSION['userId'])) {
      // code...
    }
    else{
      ...
      header("Location: searchResults.php#".$content_id);
      exit();
    }
  ...
  $users = null;
  if ($hata == false) { // Hata yoksa kullanıcıları getir
    if (!isset($_GET['search'])) { // aranan birşey yok

    }
    else{
      $users = getUsersByUsername($dbh, $_GET['search']);
    }
  }
  ...
  if ($hata == false) { // Hata yoksa gönderileri getir
    if (!isset($_GET['search'])) { // aranan birşey yok
      ...
      $contents = getContentsByTag($dbh, $_GET['search']);
    }
  }
...
<body class="bg-dark">
  ...
  ?>
  <h2> <span class="badge badge-secondary" style="margin-left: 3%;">Kullanıcılar</span> </h2>
  <!-- User Cards-->
  <div class="userCards container-fluid">

    <?php
      if ($users != null && $users->rowCount()) {
        ...
        while ($row = $users->fetch()) {
          ...
          $user->profileImg = $row['profile_img'];
          $user->drawCard();
        }
      }
      else{
  ...
  <h2> <span class="badge badge-secondary" style="margin-left: 3%;">Gönderiler</span> </h2>
  <!-- STREAM CARDS -->
...
<?php
  ...
  function getContentsByTag($dbh, $tag){
    if (!$dbh) { // DBH'da hata varsa hiçbirşey yapma
      return;
    }

    $sql = "SELECT * FROM contents WHERE LOWER(tag) LIKE concat('%', LOWER(?), '%');";
    $varArray = [$tag];
    $stmt = $dbh->select($sql, $varArray);
    if (!$stmt || !$stmt->rowCount()) { // Hata var veya hiç gönderi yok
      return false;
  ...
  function getUsersByUsername($dbh, $username){
    if (!$dbh) { // DBH'da hata varsa hiçbirşey yapma
      return false;
    }

    $sql = "SELECT * FROM users WHERE LOWER(username) LIKE concat('%', LOWER(?), '%');";
    $varArray = [$username];
    $stmt = $dbh->select($sql, $varArray);
    if (!$stmt || !$stmt->rowCount()) { // Hata var veya hiç gönderi yok
      return false;
